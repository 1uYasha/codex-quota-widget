...
最新稳定版为 `v1.4.1`，Windows 与 Apple Silicon Mac 包位于同一个 GitHub Release：

[下载 ChatGPT Quota v1.4.1](https://github.com/1nuYasha-cck/codex-quota-widget/releases/tag/v1.4.1)

Windows 下载 `.exe` 后双击运行；Apple Silicon Mac 下载 `mac-arm64.zip` 后解压并打开 `.app`。Windows 未签名，Mac 仅使用本地运行所需的 ad-hoc 临时签名，两个版本都没有受信任的商业代码签名；系统出现安全提示时请确认来源后手动放行。
...
| 平台 | 独有体验 | 发布格式 |
| --- | --- | --- |
| Apple Silicon Mac | 菜单栏显示图标与剩余额度；可选择 7 天或 5 小时；点击图标打开紧凑的 `216×390` 专属设置面板；关闭悬浮窗后菜单栏继续运行且 Dock 图标自动隐藏 | ARM64 `.zip`，解压得到 `.app` |
...
- 菜单栏设置面板采用紧凑的 `216×390` 尺寸，控件同步缩小并保留全部操作。
...
| 状态 | 颜色 | 说明 |
| --- | --- | --- |
| 正常 | 翡翠绿 `#34C98F` | 额度剩余 40% 或以上 |
| 偏低 | 琥珀黄 `#F2B84B` | 额度剩余 20%–39% |
| 紧张/用尽 | 珊瑚红 `#FF5C5C` | 额度剩余 0%–19% |
| 读取中 | 蓝色 | 正在从本机 Codex 读取快照；只影响读取状态灯 |
| 失败 | 红色 | 本机 Codex 路径、登录态或读取过程异常；只影响读取状态灯 |
...
应用输出到 `dist/mac-arm64/ChatGPT Quota.app`。应用由 `electron-builder` 生成并使用 ad-hoc 方式临时签名，以满足 ARM64 本地启动要求；该签名不是 Apple Developer ID 签名。发布 ZIP 可使用 `ditto -c -k --sequesterRsrc --keepParent` 生成。
...
更完整的架构、平台差异、构建链路和历史问题分析见 [开发与问题报告](docs/DEVELOPMENT_REPORT.md)。
